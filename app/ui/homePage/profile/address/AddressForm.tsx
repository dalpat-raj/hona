...
import { AddAddress } from "@/action/address";
...
const AddressForm: React.FC<AddressProps> = ({ userId, setOpenAddress }) => {
  ...
  return (
    <div className="bg-gray-50 p-4 max-sm:px-0">
      <div className="w-[60%] max-md:w-full">
        <h2 className="text-[16px] font-bold text-black-800 mb-2">
          Add Address
        </h2>
        <form onSubmit={handleSubmit(handleAddressSubmit)}>
          ...
          <div className="mt-4 flex gap-4 items-center">
            <div className="w-1/5 h-10">
              <ButtonWithSpinner loading={isPending}>Save Address</ButtonWithSpinner>
            </div>
            <div
              onClick={() => setOpenAddress(false)}
              className="cursor-pointer bg-bga text-[#333] border border-red-500 px-4 py-2 rounded-md transition"
            >
              Cancle
            </div>
          </div>
